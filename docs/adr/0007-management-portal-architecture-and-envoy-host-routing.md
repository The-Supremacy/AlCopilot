...
The project now needs a manager-facing portal to curate drinks catalog data and run manual import sync workflows.
We need clear frontend boundary choices that keep complexity under control while preserving future growth.
We also need AKS ingress direction aligned with the confirmed Envoy Gateway API platform and future application-level access boundaries.

## Decision

Adopt a portal-level frontend boundary:
...
- Keep management host access operationally restricted until product-level access control is implemented.
...
Host-based routing fits Envoy Gateway API cleanly and reduces future access/session ambiguity.
...
- Future access-control rollout has cleaner host-based boundary alignment.
...
Rejected for now.
Path-based routing is feasible, but host-based boundaries provide cleaner future access and operational separation for manager-only access.
