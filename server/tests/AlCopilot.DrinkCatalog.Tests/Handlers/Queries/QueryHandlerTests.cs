...
using AlCopilot.DrinkCatalog.Handlers.Queries;
using NSubstitute;
using Shouldly;
...
public sealed class GetDrinksHandlerTests
{
    ...
    public async Task Handle_ReturnsPagedResult()
    {
        ...
        _drinkRepository.GetPagedAsync(null, 1, 20, Arg.Any<CancellationToken>()).Returns(expected);

        var result = await _handler.Handle(new GetDrinksQuery(null, 1, 20), CancellationToken.None);
    ...
    public async Task Handle_WithTagFilter_PassesTagIds()
    {
        ...
        var expected = new PagedResult<DrinkDto>([], 0, 1, 20);
        _drinkRepository.GetPagedAsync(tagIds, 1, 20, Arg.Any<CancellationToken>()).Returns(expected);

        var result = await _handler.Handle(new GetDrinksQuery(tagIds, 1, 20), CancellationToken.None);
...
public sealed class SearchDrinksHandlerTests
{
    private readonly IDrinkRepository _drinkRepository = Substitute.For<IDrinkRepository>();
    private readonly SearchDrinksHandler _handler;

    public SearchDrinksHandlerTests()
    {
        _handler = new SearchDrinksHandler(_drinkRepository);
    }
    ...
    {
        var expected = new PagedResult<DrinkDto>([], 0, 1, 20);
        _drinkRepository.SearchAsync("mojito", 1, 20, Arg.Any<CancellationToken>()).Returns(expected);

        var result = await _handler.Handle(new SearchDrinksQuery("mojito", 1, 20), CancellationToken.None);
